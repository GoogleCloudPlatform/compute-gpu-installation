...
def parse_args():
    ...
    install_driver_parser.add_argument(
        "--installation-mode",
        help="Pick the installation mode. Either 'repo' or 'binary'. Repo mode will add NVIDIA repository to your "
             "sources list and install packages from repository. Binary will download binary installer files and use "
             "them to install. Default mode is 'repo'.",
        required=False,
        default="repo",
        choices=["repo", "binary"],
    )
    ...
    install_driver_parser.add_argument(
        "--installation-branch",
        help=f"Select driver branch to install from. Available branches: nfb, prod and lts. Those will install drivers "
             f"in versions {VERSION_MAP['nfb']['driver']['version']}, "
             f"{VERSION_MAP['prod']['driver']['version']} and {VERSION_MAP['lts']['driver']['version']} "
             f"respectively. Default: prod",
        required=False,
        default="prod",
    ...
    image_builder_parser.add_argument(
        "--installation-mode",
        help="Pick the installation mode. Either 'repo' or 'binary'. Repo mode will add NVIDIA repository to your "
             "sources list and install packages from repository. Binary will download binary installer files and use "
             "them to install. Default mode is 'repo'.",
    ...
    image_builder_parser.add_argument(
        "--installation-branch",
        help=f"Select driver and CUDA branch to install from. Affects only binary installation mode. "
             f"Available branches: nfb, prod and lts. "
             f"Those will install drivers in versions {VERSION_MAP['nfb']['driver']['version']}, "
             f"{VERSION_MAP['prod']['driver']['version']} and {VERSION_MAP['lts']['driver']['version']} "
             f"respectively with compatible CUDA Toolkit versions ({VERSION_MAP['nfb']['cuda']['version']}, "
             f"{VERSION_MAP['prod']['cuda']['version']} and {VERSION_MAP['lts']['cuda']['version']}). Default: prod.",
        required=False,
        default="prod",
        choices=["prod", "nfb", "lts"],
    )
    ...
    image_builder_parser.add_argument(
        "--save-keys-path",
        help="Saves the keys used to sign the installed drivers in the given location. If not provided, the keys are "
    ...
    image_builder_parser.add_argument(
        "--vm-disk-type",
        help="Type of the disk used for the image preparation. Available types: ssd, balanced and standard. "
             "Default: balanced",
        required=False,
        default="balanced",
    )
    ...
    image_builder_parser.add_argument(
        "--image-region",
        help="Location to save the disk image in. This can be a region or multi-region (us, eu or asia) name. If not "
             "provided, it will be the multiregion inferred from the --vm-zone option.",
        required=False,
        type=str,
    )
    ...
    image_builder_parser.add_argument(
        "--base-image",
        help=f"Base image to build upon. Available options: {image_builder.AVAILABLE_BASE_IMAGES}. Default: ubuntu-24",
        default="ubuntu-24",
    )
    ...
    image_builder_parser.add_argument(
        "--interactive",
        help="The image preparation process will be paused before shutting down the build VM, and SSH session will be "
             "opened. This way you can customize your future image, install additional packages etc.",
        action="store_true",
    )

    image_builder_parser.add_argument(
        "--custom-script",
        help="Provide a bash script that will be executed on the build VM before it's turned off. This way you can "
             "install additional packages and execute additional configuration steps.",
        action="store",
        type=pathlib.Path,
    )
    ...
    image_builder_parser.add_argument(
        "--subnet",
        help="Provide a VPC subnet identifier to be used for the build VM. If not provided, the name of the network "
             "will be used. Required for custom mode VPC networks.",
        required=False,
    )
    ...
    install_cuda_parser.add_argument(
        ...
        help="Pick the installation mode. Either 'repo' or 'binary'. Repo mode will add NVIDIA repository to your "
             "sources list and install packages from repository. Binary will download binary installer files and use "
             "them to install. Default mode is 'repo'. You have to use the same mode you used when installing the "
             "driver.",
        required=False,
        default="repo",
        choices=["repo", "binary"],
    ...
    install_cuda_parser.add_argument(
        ...
        help=f"Select driver and CUDA branch to install from. Affects only binary installation mode. "
             ...
             f"{VERSION_MAP['prod']['driver']['version']} and {VERSION_MAP['lts']['driver']['version']} "
             f"respectively with compatible CUDA Toolkit versions ({VERSION_MAP['nfb']['cuda']['version']}, "
             f"{VERSION_MAP['prod']['cuda']['version']} and {VERSION_MAP['lts']['cuda']['version']}). \n\n"
             f"This value must match the value used for driver installation, if driver was installed separately. \n"
             f"Default: prod.",
        required=False,
        default="prod",
        choices=["prod", "nfb", "lts"],
